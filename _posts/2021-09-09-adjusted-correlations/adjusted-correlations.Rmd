...
dat_stacked %>% 
  ...
  geom_point(aes(color = playlist_genre), alpha = 0.2, show.legend = FALSE) +
  facet_grid(attribute ~ music_category, scales = "free") +
  scale_color_manual(values = colors_genre) +
  theme_shannon()
```
...
results_2 <- dat_stacked %>%
  ...
  nest() %>% 
  mutate(
    corr = map(data, compute_adj_corr, var_dep = "danceability", var_ind = "value", var_group = "playlist_genre")
  ) %>% 
...
results_all <- results_1 %>% 
  ...
  mutate_if(is.numeric, scales::number, 0.01) %>% 
