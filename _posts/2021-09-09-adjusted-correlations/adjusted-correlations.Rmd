...
results_1 <- dat_stacked %>%
  ...
  ungroup() %>% 
  unnest(c(corr)) %>% 
  dplyr::select(music_category, attribute, corr = estimate)
```
...
results_2 <- dat_stacked %>%
  ...
  dplyr::select(music_category, attribute, corr)
```
...
```{r}
dat_penguins %>% 
  bind_rows(dat_penguins %>% mutate(species = "All")) %>% 
  group_by(species) %>% 
  nest() %>% 
  mutate(
    base_n = map_int(data, nrow),
  ...
  unnest(c(data, corr)) %>% 
  mutate(
    species = fct_relevel(species, "Gentoo", "Chinstrap", "Adelie", "All"),
    corr_label =  glue::glue("{species}\nn = {base_n}\n r = {scales::number(estimate, 0.01)}"),
    corr_label = fct_reorder(as.character(corr_label), as.numeric(species))
   ) %>% 
