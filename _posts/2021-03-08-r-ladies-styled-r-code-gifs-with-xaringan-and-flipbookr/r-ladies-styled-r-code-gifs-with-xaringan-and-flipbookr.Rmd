---
title: "R-Ladies styled R code gifs with xaringan and flipbookr"
description: |
...
![](gifs/demo-walrus-flipbook-quality.gif){width=70%}
...
The `flipbookr` [template](https://github.com/EvaMaeRey/flipbookr#template){target="_blank"} makes it easy to quickly create content! I actually only used the classic flipbook, with defaults to walk through the code pipeline (slides 3-4 in the template).
...
My first attempt to style the content used Alison Hill's [R-Ladies xaringan theme](https://alison.rbind.io/project/rladies-xaringan/){target="_blank"} by declaring
...
![](img/walrus-default.PNG){width=70%}
...
![](walrus-adjusted.PNG){width=70%}

The changes are subtle, but I hope they improve readability! Silvia showed me how to 
modify the CSS through (1) inclusion of a CSS code chunk, and (2) calling a separate
CSS sheet. The increased font size did create a challenge for code formatting and
for the amount of code that could be shown on a single slide.

You can see the full code for the default xaringan theme and the two ways of adjusting the
CSS for all the gifs I created in the  [rladies-flipbookr](https://github.com/shannonpileggi/rladies-flipbookr){target="_blank"} github repository.
...
content! However, please remember to share the content with _everyone_. Gifs and images on their own are not accessible to members of our community that use assistive technology to read digital content. In order to make gifs and images accessible to screen readers, you can include [descriptions](https://help.twitter.com/en/using-twitter/picture-descriptions){target="_blank"} or share code in otherwise accessible formats like [GitHub gists](https://gist.github.com/){target="_blank"}. If I miss making
any of my content accessible, please let me know!


# Acknowledgements

I have to give [Silvia Canelón](https://silvia.rbind.io/){target="_blank"} ([`@spcanelon`](https://twitter.com/spcanelon){target="_blank"}) **huge** shout out here. My curating week started on a Monday, and on Monday night I was still trying to figure out how 
I wanted to style my code. We chatted on [R-Ladies slack](https://rladies-community-slack.herokuapp.com/){target="_blank"}, and she quickly helped me make CSS adjustments to the default xaringan R-Ladies theme. After the curating week, Silvia helped me fine tune the bits to share that are now in the [rladies-flipbookr](https://github.com/shannonpileggi/rladies-flipbookr){target="_blank"} repo. Additionally, she advised me on accessibility best practices. Thank you, Silvia! `r emo::ji("purple heart")` `r emo::ji("pray")`

# Outtakes 

I experimented with, and actually recorded, some not so compelling formats before 
I landed on `flipbookr`. Here are two gifs that didn't make the cut! `r emo::ji("see-no-evil monkey")`

### Walrus in RMarkdown

![](gifs/demo-walrus-rmarkdown.gif){width=70%}

### Walrus in console

![](gifs/demo-walrus-console.gif){width=70%}
