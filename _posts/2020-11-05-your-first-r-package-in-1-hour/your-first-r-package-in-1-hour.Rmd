...
<aside>
Artwork adapted from [`@allison_horst`](https://twitter.com/allison_horst?lang=en).
</aside>
...
3. Creates the minimal essential files and structure required for R packages, as outlined in this image from the [package development](https://rawgit.com/rstudio/cheatsheets/master/package-development.pdf) cheat sheet.

<aside>
We discuss the file structure in the workshop.
</aside>

![](package_structure.PNG){width=40%}



# First check

Now that we have a package, let's check it.  Submitting [`devtools::check()`](https://r-pkgs.org/r-cmd-check.html) updates package documentation, builds the package, and submits over 50 checks for metadata, structure, R code, documentation, and more! 
