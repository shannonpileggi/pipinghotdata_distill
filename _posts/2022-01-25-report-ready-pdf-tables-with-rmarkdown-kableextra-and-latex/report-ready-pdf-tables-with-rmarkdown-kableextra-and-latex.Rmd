...
folder <- "2022-01-25-report-ready-pdf-tables-with-rmarkdown-kableextra-and-latex"
```
...
and a bit of LaTeX. Recent updates available in
...
Here is a solution have landed upon; I hope it helps you and others as
...
-   `stripe_color = "gray!15"` species the stripe color using LaTeX color
    from the [xcolor
...
With the latest release of [`gtsummary`
...
2.  You can pass the same options to `gtsummary::as_kable_extra()` that
    can be passed to `knitr::kable()` / `kableExtra::kbl()`.

3.  Finish up with additional `kableExtra::kable_styling()`
    specifications.
...
1.  Create a `gtsummary` table object. Here, I also modify the default
    statistic printed with extra escaping characters so that I can use
    `escape = FALSE` for custom line breaks.
...
2.  Retrieve and modify column names with line breaks and bold text.

```{r}
tbl_names <- tbl$table_styling$header %>% 
  filter(str_detect(column, "stat|^label")) %>% 
  select(label) %>% 
  mutate(
    row_1 = str_extract(label, "\\*\\*.*\\*\\*") %>% 
      str_remove_all(pattern = fixed("**")) %>% 
      sprintf("\\textbf{%s}", .),
    row_2 = str_remove_all(label, ".*\\*\\*") %>% 
      str_remove(", "),
    styled_names = glue::glue("{row_1}\n{row_2}")
  ) %>% 
  pull(styled_names) %>% 
  as.character()
...
3.  Print table with same options previously used.
...
tbl %>% 
  gtsummary::as_kable_extra(
    ...
    longtable = TRUE,
    linesep = "",
    col.names = linebreak(tbl_names, align = "l"),
    escape = FALSE
    ) %>%
  kableExtra::kable_styling(
      position = "left",
