---
title: "Locating R Software and Configuration Files on Windows 10"
description: |
  My personal R administration
author:
...
#| fig.cap: > 
#|   File tree visualizing the hierarchy of R and R adjacent software and configuration files on my Windows 10 OS.
#| fig.alt: > 
#|    Two home folders are colored bright purple, the software folders are colored bright green,
...
paths <- readxl::read_excel("C:/Users/pileggis/Documents/gh-personal/pipinghotdata_distill/_posts/2022-05-31-personal-r-configuration-on-windows/locations.xlsx") %>% 
  mutate(
...
Here is what my Windows 10 set up looks like, with associated code if available.
...
Wrapping the function in `fs::path_real()` allows you to see the full path, 
rather than the [short file path](https://en.wikipedia.org/wiki/8.3_filename){target="_blank"}.
...
`.libPaths()` shows where R is looking for packages in the current session. 

On Windows, with the default configuration in R versions prior to 4.2.0, you may have more than one path present, which
correspond to a directory specific to the user and a directory available to all users.
If more than one `.libPaths()` is present, R attaches packages from the libraries
...
My R packages were initially being installed to a default location at `fs::path_home_r()`; 
I modified this location to my `C` drive. You can modify the default location using 
the [`.Renviron`](#renviron) configuration file.
...
1. R 4.2.0 was released on 2022-04-22. As described in Jumping Rivers
...
directory. When I work in dev mode, the development packages get
