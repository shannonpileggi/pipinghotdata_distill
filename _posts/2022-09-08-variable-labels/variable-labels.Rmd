...
use the `penguins` data set from the {palmerpenguins} package.
...
Now assign the labels using the splice operator. Using the [splice operator](https://rlang.r-lib.org/reference/splice-operator.html){target="_blank"}, 
...
It is very convenient to search this within the RStudio data viewer to find variables of interest.
...
I wish there was a simpler solution here where you could supply the `penguins_labels`
vector to `labs()` and have everything automatically line up.



## 5. Exporting data with variable labels

I have an underdeveloped public facing work package [{croquet}](https://github.com/pcctc/croquet){target="_blank"}
which contains a single function we use all the time: `add_labelled_sheet()`. This function
exports data to excel with row 1 as variable names, and row 2 as the descriptive
...
Exporting data with labels does feel unnatural to me as the data is 
...
I found the ggplot labelling strategy in Lefkios Paikousis's blog post ["A workflow with labelled data"](https://lefkiospaikousis.netlify.app/posts/2020-12-23-a-workflow-with-labelled-data/){target="blank"},
which has other labelling tips for value labels as well!
