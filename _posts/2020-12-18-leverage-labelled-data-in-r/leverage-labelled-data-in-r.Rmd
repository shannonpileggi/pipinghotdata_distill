...
with SAS, SPSS, and Stata data sets in R. The `haven_labelled` class stores variable and value labels. Challenges incurred by the `haven_labelled` class can be easily resolved with conversion to `numeric` and `factor` classes. Here are my most used functions for getting started with labelled data:
...
and how the metadata of `q8` is stored as attributes, similar in spirit to factors.  
...
console simultaneously prints value codes and labels.
...
I primarily use three packages for working with labelled data: `haven`, `labelled`, and `sjlabelled`. These three packages do have some overlap in functionality, in addition to naming schemes that differ but achieve the same objective (e.g., `haven::as_factor` vs `sjlabelled::as_label`), or naming schemes that are the same but achieve different objectives (e.g., `haven::as_factor` vs `sjlabelled::as_factor`). `r emo::ji("grimace")` To compound confusion, the concept of a _label_ can refer to either a 
variable label or value labels. Frequently, function names refer to value labels in the plural, as in `haven::zap_labels` or `labelled::remove_val_labels`. 
...
2. Convert `haven_labelled` variable to `numeric` value codes.
   ...
   + _Function(s):_ `base::as.numeric()` (strips variable of all metadata/attributes), `haven::zap_labels()` and `labelled::remove_val_labels` (removes value labels, retains other metadata/attributes) 
...
This code produces a barplot showing the frequencies of the three variables from the original labelled data.
...
dat_2017 %>% 
  ...
  facet_wrap(. ~ variable) +
  geom_col() +
  ggtitle("Bar plot 1: Displaying variable names and value codes")
```

Now I add two lines of code to implement two changes - convert the variables to factors and convert the variable labels to variable names. I do not claim that this barplot is beautiful, but
it is readable!
...
dat_2017 %>% 
  ...
  facet_wrap(. ~ variable) +
  geom_col() +
  ggtitle("Bar plot 2: Displaying variable labels and value labels")
```
...
For example, in using `ggplot` for `bar plot 1` above, I included the line `y = factor(days)`; if instead I had `y = days` in `bar plot 1`, `ggplot` yields the following message: 
...
where the conversion happens in your workflow. The [Intro to labelled](https://cran.r-project.org/web/packages/labelled/vignettes/intro_labelled.html) vignette
...
2. First perform data utilizing variable codes; second convert `haven_labelled` variables.  

For me this question usually distills down to: for data manipulation, are the value codes or the value labels easier to work with? Sometimes the brevity of the value code helps, whereas other times the context of the value label makes the programming more readable. Note that the placement of the conversion can have downstream effects on your code. 
