...
with SAS, SPSS, and Stata data sets in R through implementation of the `haven_labelled` class, which stores variable and value labels. Challenges incurred by the `haven_labelled` class can be easily resolved with conversion to `numeric` and `factor` classes. Here are my most used functions for getting started with labelled data:
...
intentionally using an older version due to an issue I encountered with 2.7.0 and the 
`look_for` function.  You can [install](https://support.rstudio.com/hc/en-us/articles/219949047-Installing-older-versions-of-packages){target="_blank"} older versions of a package.
</aside>
...
The `haven`, `labelled`, `sjlabelled` packages create new structures and work flows for labelled data that allow you to harness the power of R while still honoring the valuable metadata framework that exists in SPSS, SAS, and Stata. The functions discussed in this post cover most of my daily needs with labelled data; if you want to do more, next steps might include handling specific types of coded missing data or creating labelled data within R.
