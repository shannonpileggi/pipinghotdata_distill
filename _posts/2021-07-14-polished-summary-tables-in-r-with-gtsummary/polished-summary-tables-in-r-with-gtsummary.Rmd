...
This blog post is to promote `gtsummary` and make it more searchable. The `gtsummary` documentation is excellent so I won't cover all of its awesome functionality, but I will add a bit of my specific experience. 
...
dat <- dat_raw %>% 
  ...
  # a few variables
  dplyr::select(record, year, q12, stheight) 
```
...
dat %>% 
  ...
  mutate_at(vars(q12), haven::as_factor) %>% 
...
And *wait* - did you see that?! The raw data had variable names of `q12`, `stheight`, but the table printed the variable label! (I previously [tweeted](https://twitter.com/WeAreRLadies/status/1363186751133667330?s=20){target="_blank"} about the awesome package pairing of `haven` and `gtsummary`.) If your data does not come with handy labels, you can create them with the [`label` option in `tbl_summary`](http://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html){target="_blank"} or with the [`var_label` function](https://larmarange.github.io/labelled/reference/var_label.html){target="_blank"} in the `labelled` package.
...
From this summary table, I can infer that there are 14,765 records in the year 2017; 11,541 of them have non-missing values and 3,224 have missing values. The percents shown in the table are the percent of the non-missing base n; that is, 5,462 / 11,541 is 47%.

You can remove printing of the count of unknown values if you like, which does not change any of other numbers in your table.
...
If you want your `n` at the top of the column to reflect the n for non-missing observations, I would remove those in your data cleaning process.
...
On the other hand, if the missing data represented a valid category that you wanted counted, you could replace the missing values (which would shift the percentages).
...
dat_raw %>% 
  ...
  mutate_at(vars(q69), fct_explicit_na, "Missing weight action") %>% 
  gtsummary::tbl_summary()
```

# Limitations

There really aren't many! The `gtsummary` package has readable output that is easily customizable. The one hang up for me is that I work with many who prefer to see tables in excel, and this an output not currently supported in `gtsummary` (see [supported output types](http://www.danieldsjoberg.com/gtsummary/index.html#gtsummary--r-markdown){target="_blank"}). No shade here, I'm waiting patiently for this to happen in the [`gt` package](https://github.com/rstudio/gt/issues/240){target="_blank"}. `r emo::ji("grin")`

# Acknowledgements

Daniel Sjoberg + team rocks for their work on `gtsummary`. Thanks to new learneRs who inspire me to write. `r emo::ji("purple heart")`